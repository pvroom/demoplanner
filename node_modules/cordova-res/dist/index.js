...
async function CordovaRes({ directory = process.cwd(), resourcesDirectory = resources_1.DEFAULT_RESOURCES_DIRECTORY, logstream = process.stdout, errstream = process.stderr, platforms = {
    ...
    await config_1.run(configPath, resourcesDirectory, sources, resources);
    ...
    return {
        ...
        sources: sources.map(source => {
            switch (source.type) {
                case "raster" /* RASTER */:
                    return { type: "raster" /* RASTER */, value: source.src };
                case "color" /* COLOR */:
                    return { type: "color" /* COLOR */, name: source.name, value: source.color };
            }
        }),
    };
}
